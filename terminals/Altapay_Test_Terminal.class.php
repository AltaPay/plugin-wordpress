    ...
    */
	...
	$config = array(
        ...
        try {
            ...
            if (is_null($savedCardNumber)) {
                ...
                $results
                    = $wpdb->get_results("SELECT ccToken FROM {$wpdb->prefix}altapayCreditCardDetails WHERE creditCardNumber='$savedCardNumber'");
                foreach ($results as $result) {
                    $ccToken = $result->ccToken;
                }
            }
            $response      = $api->createPaymentRequest($terminal, $order_id, $amount, $currency, $payment_type,
                $customerInfo, $cookie, $language, $config, $transactionInfo, $orderLines, false, $ccToken, null, null,
                null, null, null, $customerCreatedDate);
    ...
    public function checkAltapayResponse()
    {
        ...
        if ($_GET['wc-api'] === 'WC_Gateway_' . $this->id) {
            global $woocommerce;
            $postdata    = $_POST;
            $xmlResponse = preg_replace('/\\\\/', '', $_POST['xml']);
            $xml         = new SimpleXMLElement($xmlResponse);
            $xmlToJson   = json_encode($xml->Body->Transactions->Transaction);
		...
		exit;
            ...
            if ($postdata['type'] === 'paymentAndCapture' && $postdata['require_capture'] == 'true') {
                $api = $this->apiLogin();
