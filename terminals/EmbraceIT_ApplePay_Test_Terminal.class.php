...
class WC_Gateway_EmbraceIT_ApplePay_Test_Terminal extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id					= strtolower('altapay_EmbraceIT_ApplePay_Test_Terminal');
		$this->has_fields			= false;
		$this->method_title			= 'AltaPay - EmbraceIT ApplePay Test Terminal';
		...
		$this->terminal				= 'EmbraceIT ApplePay Test Terminal';
...
//
//        error_log('process_payment triggered');
//
//
//        return array(
//            'result'   => 'success',
...
//    }
	...
	public function admin_options() {
		echo '<h3>EmbraceIT ApplePay Test Terminal</h3>';
...
?>
            <script src="https://applepay.cdn-apple.com/jsapi/v1/apple-pay-sdk.js?ver=1.0.0" id="altapay-applepay-sdk-js"></script>
            <script>
            var applepay_ajax_obj = {
                'ajax_url' : '<?php echo admin_url( 'admin-ajax.php' ); ?>',
                'nonce' : '<?php echo wp_create_nonce( 'apple-pay' ); ?>',
                'currency' : '<?php echo get_woocommerce_currency(); ?>',
                'country' : '<?php echo get_option( 'woocommerce_default_country' ); ?>',
                'cart_totals' : '<?php

                    $cart_totals = WC()->session->get( 'cart_totals', null );

                    echo $cart_totals['subtotal'];
                ?>',
                'apple_pay_terminal' : '<?php

                        $applePayObj = new Core\ApplePay();
                        echo $applePayObj->filter_apple_apy_terminals( WC()->payment_gateways->payment_gateways() );
                    ?>',
            }
            </script>
            <script src="https://wpdev.altapayplatform.com/wp-content/plugins/plugin-wordpress/assets/js/applepay.js?ver=1.0.0" id="altapay-applepay-main-js"></script>
            <script type="text/javascript">
                jQuery( document ).ready(
                    function($) {
                        onApplePayButtonClicked(applepay_ajax_obj);
                    }
                );
